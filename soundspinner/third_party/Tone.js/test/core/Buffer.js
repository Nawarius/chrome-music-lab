define(["Test", "Tone/core/Buffer"], function (Test, Buffer) {
	...
	describe("Buffer", function(){
		it ("can be created and disposed", function(){
			var buff = new Buffer("./soundspinner/audio/kick.mp3");
			buff.dispose();
			Test.wasDisposed(buff);
		});

		it("loads a file from a url string", function(done){
			var buffer = new Buffer("./soundspinner/audio/kick.mp3", function(buff){
		...
		it("has a duration", function(done){
			var buffer = new Buffer("./soundspinner/audio/kick.mp3", function(){
		...
		it("the static on('load') method is invoked", function(done){
			var buffer = new Buffer("./soundspinner/audio/hh.mp3");
		...
		it("takes Tone.Buffer in the set method", function(done){
			var buffer = new Buffer({
				"url" : "./soundspinner/audio/hh.mp3",
				"onload" : function(){
					var testOne = new Buffer("./soundspinner/audio/hh.mp3");
					testOne.set(buffer);
					expect(testOne.get()).to.equal(buffer.get());
					testOne.dispose();
					buffer.dispose();
